...
namespace Client.MirControls
{
    public sealed class MirGoodsCell : MirControl
    {
        ...
        private void Update()
        {
            ...
            if (UsePearls)
            {
                PriceLabel.Text = string.Format("售价: {0} 珍珠{1}", (uint)(Item.Price() * GameScene.NPCRate), Item.Price() > 1 ? "s" : "");
            }
            else if (Recipe)
            {
                ClientRecipeInfo recipe = GameScene.RecipeInfoList.SingleOrDefault(x => x.Item.ItemIndex == Item.ItemIndex);

                PriceLabel.Text = string.Format("售价: {0} 金币", (uint)(recipe.Gold * GameScene.NPCRate));
            }
            else
            {
                PriceLabel.Text = string.Format("售价: {0} 金币", (uint)(Item.Price() * GameScene.NPCRate));
            }
        }
