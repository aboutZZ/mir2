    ...
    public class NPCObject : MapObject
    {
        ...
        public override void DrawName()
        {
            ...
            for (int s = 0; s < splitName.Length; s++)
            {
                ...
                // 【NPC名字】这里原来是 s * 12 改为 s * 18, NPC名字有时候两行, 每行间距调宽点
                TempLabel.Location = new Point(DisplayRectangle.X + (48 - TempLabel.Size.Width) / 2, DisplayRectangle.Y - (32 - TempLabel.Size.Height / 2) + (Dead ? 35 : 8) - (((splitName.Length - 1) * 10) / 2) + (s * 18));
                TempLabel.Draw();
            }
        }
